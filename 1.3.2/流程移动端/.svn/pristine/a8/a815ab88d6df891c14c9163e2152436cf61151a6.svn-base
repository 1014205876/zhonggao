...
export default {
  ...
  created() {
    ...
    for (let i = 0; i < 7; i++) {
      // 将时间格式化（防止跨月出现11.0-6这种）
      let date01 = new Date();
      let year01 = date01.getFullYear();
      let month01 = date01.getMonth();
      let day01 = date01.getDate() - 7 + i;
      // 将格式化好的时间用new Date(2018,11(12月),-1(上个月的倒数第二天))的方式取到正确时间
      let date = new Date(year01,month01,day01);
